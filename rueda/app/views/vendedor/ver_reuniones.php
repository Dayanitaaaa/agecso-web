    ...
    <div class="space-y-8">
        ...
        <?php if (!empty($_GET['msg'])): ?>
            <?php if ($_GET['msg'] === 'contraoferta_enviada'): ?>
                <div class="bg-gradient-to-r from-teal-50 to-emerald-50 border-2 border-teal-300/40 p-5 rounded-3xl shadow-sm flex items-center justify-between animate-modal">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-sm font-black text-gray-900 flex items-center gap-2">
                                ...
                                <span class="bg-teal-100 text-[#0d9488] text-[10px] px-2.5 py-0.5 rounded-full font-black uppercase tracking-wider">En Negociación</span>
                            </h3>
                            <p class="text-xs text-gray-600 mt-0.5 font-medium">Tu propuesta de nueva fecha y hora ha sido notificada al comprador.</p>
                        </div>
                    </div>
                ...
                <div class="bg-gradient-to-r from-emerald-50 to-teal-50 border-2 border-emerald-300/40 p-5 rounded-3xl shadow-sm flex items-center justify-between animate-modal">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-600 mt-0.5 font-medium">La reunión ha quedado formalmente programada.</p>
                        </div>
                    </div>
                    <button onclick="this.parentElement.remove()" class="text-gray-400 hover:text-gray-600 p-2 rounded-xl hover:bg-white/60 transition-colors">
                        <i class="fas fa-times"></i>
        ...
        <!-- SELECTOR DE RUEDA -->
        <?php if (!empty($ruedas)): ?>
        <div class="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-gray-100 p-6 sm:p-7">
            <div class="flex items-center justify-between gap-4 mb-4">
                <div class="flex items-center gap-2.5">
                    <div class="w-9 h-9 rounded-xl bg-teal-50 text-[#0d9488] flex items-center justify-center shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
                ...
                ?>
                    <a href="index.php?controlador=vendedor&accion=verReuniones&rueda_id=<?php echo $r['id']; ?>" 
                       class="group p-4 rounded-2xl border-2 transition-all duration-300 flex items-center justify-between gap-3 text-left <?php echo $isSelected ? 'border-[#0d9488] bg-teal-50/50 shadow-sm ring-2 ring-teal-500/20' : 'border-gray-200 bg-white hover:border-teal-300 hover:bg-gray-50/60 shadow-sm'; ?>">
                        <div class="flex items-center gap-3 min-w-0">
                            <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all <?php echo $isSelected ? 'bg-[#0d9488] text-white shadow-md shadow-teal-500/20' : 'bg-teal-50 text-[#0d9488] group-hover:bg-[#0d9488] group-hover:text-white'; ?>">
                                <i class="fas fa-calendar-check text-sm"></i>
                            </div>
                            <div class="min-w-0">
                                <h4 class="text-sm font-extrabold text-gray-900 truncate group-hover:text-[#0d9488] transition-colors"><?php echo htmlspecialchars($nombreRueda); ?></h4>
                                <div class="flex items-center gap-2 mt-1">
                                    <span class="text-[11px] font-bold text-gray-500 flex items-center gap-1">
                                        <i class="far fa-clock text-[10px] text-teal-600"></i> <?php echo $fechaInicioFmt; ?>
                        ...
                        <div class="shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all <?php echo $isSelected ? 'border-[#0d9488] bg-[#0d9488] text-white' : 'border-gray-300 bg-white text-transparent group-hover:border-teal-400'; ?>">
            ...
            <?php if ($rueda_actual && $total_citas > 0): ?>
                <div class="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500 font-bold flex-wrap gap-2">
                    <span class="flex items-center gap-1.5 text-[#0d9488]">
                    ...
                    <span class="bg-teal-50 text-[#0d9488] px-3 py-1 rounded-full border border-teal-100 font-black">
                ...
                <div class="p-5 space-y-5 max-h-[600px] overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <?php else: ?>
                        <?php foreach ($citas_programadas as $cita): ?>
                            ...
                            ?>
                            <div class="bg-white p-5 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.03)] border <?php echo $tiempoCumplido ? 'border-teal-100 bg-teal-50/10' : ($enCurso ? 'border-emerald-200 ring-2 ring-emerald-100' : 'border-gray-100'); ?> hover:border-emerald-200 transition-all duration-300 transform hover:-translate-y-1">
                                <div class="flex justify-between items-start mb-3">
                                    <div>
                                        <p class="font-extrabold text-gray-900"><?php echo htmlspecialchars($cita['nombre_comprador'] ?? 'N/A'); ?></p>
                                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1"><?php echo htmlspecialchars($cita['tituloRueda'] ?? 'N/A'); ?></p>
                                ...
                                <div class="text-xs text-gray-700 font-bold bg-gray-50 px-3 py-2 rounded-xl border border-gray-100 inline-flex items-center gap-2 mb-4">
                                    <i class="far fa-calendar-alt text-[#0d9488]"></i> 
                                    <span><?php echo date('d/m/Y', $fechaInicioTs); ?></span>
                                    <span class="text-gray-300">|</span>
                                    <span class="text-[#0f766e] font-black"><?php echo $horaInicioStr; ?> - <?php echo $horaFinStr; ?></span>
                                    <span class="text-[10px] text-gray-400 font-bold">(<?php echo $duracion; ?> min)</span>
                                </div>

                                <?php if ($tiempoCumplido): ?>
                                    <!-- Alerta informativa de tiempo de cita cumplido -->
                                    <div class="bg-teal-50/90 border border-teal-100 rounded-2xl p-3 flex items-start gap-2.5 text-teal-900 mb-3">
                                        <i class="fas fa-info-circle text-[#0d9488] text-sm mt-0.5"></i>
                                        <div class="text-xs">
                                            <p class="font-extrabold text-teal-950">¡Tiempo estimado cumplido (<?php echo $duracion; ?> min)!</p>
                                            <p class="text-[11px] text-teal-700 font-medium mt-0.5 leading-relaxed">
                                                Esta reunión ha concluido. Registra el resultado del acuerdo para la trazabilidad comercial de la rueda.
                                            </p>
                                        </div>
                                    </div>
                                <?php endif; ?>
                                ...
                                <div class="flex flex-col gap-2">
                                    ...
                                    $tituloEvento = urlencode("Reunión AGECSO: " . ($cita['nombre_comprador'] ?? 'Socio'));
                                ...
                                <?php if ($tiempoCumplido): ?>
                                    <button onclick="abrirModalEncuesta(<?php echo $cita['id']; ?>, '<?php echo addslashes(htmlspecialchars($cita['nombre_comprador'] ?? 'Socio')); ?>', '<?php echo date('d/m/Y H:i', $fechaInicioTs); ?>', '<?php echo addslashes(htmlspecialchars($cita['tituloRueda'] ?? 'Rueda')); ?>', 'satisfaccion')" 
                                            class="w-full bg-[#0d9488] hover:bg-[#0f766e] text-white py-3 rounded-2xl text-xs font-black transition-all duration-300 shadow-md shadow-teal-500/20 uppercase tracking-wider flex items-center justify-center gap-2 mt-2">
                                        <i class="fas fa-chart-line"></i> Registrar Resultado
                                    </button>
                                <?php elseif ($ahora >= $fechaInicioTs): ?>
                                    <button onclick="abrirModalEncuesta(<?php echo $cita['id']; ?>, '<?php echo addslashes(htmlspecialchars($cita['nombre_comprador'] ?? 'Socio')); ?>', '<?php echo date('d/m/Y H:i', $fechaInicioTs); ?>', '<?php echo addslashes(htmlspecialchars($cita['tituloRueda'] ?? 'Rueda')); ?>', 'satisfaccion')" 
                                            class="w-full bg-[#0f766e] hover:bg-[#0d9488] text-white py-2.5 rounded-xl text-xs font-black transition-all duration-300 shadow-md shadow-teal-500/10 uppercase tracking-wider flex items-center justify-center gap-2 mt-2">
                                        <i class="fas fa-chart-line"></i> Registrar Resultado
                                    </button>
                                <?php endif; ?>
                            </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (msg === 'contraoferta_enviada') {
        Swal.fire({
            ...
            text: 'Tu nueva propuesta de fecha y hora fue enviada con éxito al comprador.',
