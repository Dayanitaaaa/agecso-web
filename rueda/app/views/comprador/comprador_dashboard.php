...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="space-y-10">
        ...
        <div class="bg-gradient-to-r from-[#00a2ff] via-[#4dbfff] to-[#008ae0] rounded-[2.5rem] p-8 sm:p-10 shadow-[0_15px_40px_rgba(0,162,255,0.2)] text-white relative overflow-hidden flex flex-col md:flex-row md:items-center md:justify-between gap-8">
            ...
            <div class="relative z-10 grid grid-cols-2 gap-4">
                <a href="index.php?controlador=comprador&accion=verReuniones" class="bg-white/10 hover:bg-white/20 backdrop-blur-lg border border-white/30 p-4 rounded-3xl transition-all duration-300 group">
                ...
                <a href="index.php?controlador=usuario&accion=perfil" class="bg-white/10 hover:bg-white/20 backdrop-blur-lg border border-white/30 p-4 rounded-3xl transition-all duration-300 group">
                    <i class="fas fa-user-circle text-2xl mb-2 group-hover:scale-110 transition-transform"></i>
                    <p class="text-xs font-black uppercase tracking-wider">Mi Perfil</p>
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <a href="index.php?controlador=comprador&accion=seleccionarRuedaParticipantes" class="group bg-white p-8 rounded-[2rem] border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.02)] hover:shadow-[0_20px_50px_rgba(0,162,255,0.1)] transition-all duration-500 flex flex-col items-center text-center transform hover:-translate-y-2">
                ...
                <div class="mt-6 flex items-center text-[#00a2ff] font-black text-xs uppercase tracking-widest">
                    Ir al Catálogo <i class="fas fa-arrow-right ml-2 group-hover:translate-x-1 transition-transform"></i>
                </div>
            </a>
                ...
                <?php else: ?>
                    <div class="grid grid-cols-1 gap-6">
                        <?php foreach ($ruedas_activas as $r): 
                            ...
                            $totalDemandas = count($demandas_por_rueda[$r['id']] ?? []);
                            $totalCitas = count($citas_por_rueda[$r['id']] ?? []);
                        ?>
                            <div class="bg-white rounded-3xl shadow-[0_4px_25px_rgba(0,0,0,0.03)] border border-gray-100 overflow-hidden hover:shadow-[0_10px_35px_rgba(0,0,0,0.06)] transition-all duration-300">
                                    ...
                                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2 flex-wrap mb-2.5">
                                                ...
                                                <?php else: ?>
                                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-sky-100 text-sky-800 border border-sky-200 shadow-sm">
                                            ...
                                            <div class="mt-3 flex items-center gap-3 text-xs text-gray-500 font-semibold flex-wrap">
                                                ...
                                                <?php else: ?>
                                                    <span class="inline-flex items-center gap-1.5 text-slate-700 bg-slate-100 px-3 py-1 rounded-xl">
                                                        <i class="far fa-calendar-alt text-sky-600"></i> Evento: <?php echo date('d/m/Y', strtotime($r['fechaInicio'])); ?> al <?php echo date('d/m/Y', strtotime($r['fechaFin'])); ?>
                                                    </span>
                                        ...
                                        <div class="flex items-center gap-4 bg-white p-3.5 rounded-2xl border border-gray-200 shadow-sm shrink-0 self-start md:self-center">
                                            <div class="text-center px-2">
                                                <p class="text-[10px] font-extrabold text-gray-400 uppercase tracking-wider">Demandas</p>
                                                <p class="text-lg font-black text-gray-800"><?php echo $totalDemandas; ?></p>
                                            </div>
                                            <div class="w-px h-8 bg-gray-200"></div>
                                            <div class="text-center px-2">
                                                <p class="text-[10px] font-extrabold text-gray-400 uppercase tracking-wider">Citas</p>
                                                <p class="text-lg font-black text-sky-600"><?php echo $totalCitas; ?></p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="p-4 sm:px-6 bg-white flex flex-wrap items-center justify-between gap-2.5">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <a href="index.php?controlador=comprador&accion=verParticipantes&id=<?php echo $r['id']; ?>" class="inline-flex items-center gap-2 bg-sky-600 hover:bg-sky-700 text-white text-xs font-black px-5 py-2.5 rounded-xl transition duration-200 shadow-md shadow-sky-500/20">
                                            <i class="fas fa-search-dollar text-[11px]"></i> Explorar Ofertas
                                        </a>
                                        <button onclick="abrirModalDemanda(<?php echo $r['id']; ?>)" class="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-600 hover:text-white border border-emerald-200 text-xs font-black px-4 py-2.5 rounded-xl transition duration-200">
                                            <i class="fas fa-plus text-[10px]"></i> Publicar Demanda
                                        </button>
                                        <?php if (($r['modalidad'] ?? 'virtual') !== 'virtual' && ($r['cantidadMesas'] ?? 0) > 0): ?>
                                            <a href="index.php?controlador=comprador&accion=apartarMesa&id=<?php echo $r['id']; ?>" class="inline-flex items-center gap-1.5 bg-orange-50 text-orange-700 hover:bg-orange-600 hover:text-white border border-orange-200 text-xs font-black px-4 py-2.5 rounded-xl transition duration-200">
                                                <i class="fas fa-chair text-[10px]"></i> Reservar Mesa
                                            </a>
                                        <?php endif; ?>
                                    </div>
                                    <div>
                                        <a href="index.php?controlador=comprador&accion=verReuniones" class="inline-flex items-center gap-1.5 bg-slate-100 text-slate-700 hover:bg-slate-200 text-xs font-black px-4 py-2.5 rounded-xl transition duration-200">
                                            <i class="far fa-calendar-check text-[11px] text-sky-600"></i> Mi Agenda
                                        </a>
                                    </div>
                                </div>
                            </div>
