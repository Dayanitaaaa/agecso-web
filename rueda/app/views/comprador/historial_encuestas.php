...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="space-y-10">
        ...
        <div class="bg-gradient-to-r from-[#00a2ff] via-[#4dbfff] to-[#008ae0] rounded-3xl p-6 sm:p-8 shadow-[0_10px_30px_rgba(0,162,255,0.15)] text-white relative overflow-hidden flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            ...
            <div class="relative z-10">
                ...
                <p class="text-white/90 mt-2 flex items-center text-sm sm:text-base font-medium">
                    <i class="fas fa-star-half-alt mr-2 text-white/80"></i> Revisa tus calificaciones otorgadas y encuestas pendientes
                </p>
            </div>
        </div>
        ...
        <?php if (!empty($encuestas_pendientes)): ?>
            <div class="mb-14">
                <h2 class="text-sm font-black text-[#00a2ff] mb-6 flex items-center gap-3 uppercase tracking-[0.2em]">
                    <i class="fas fa-clock text-sky-400"></i> Pendientes por calificar
                </h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php foreach ($encuestas_pendientes as $ep): ?>
                        <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 border-l-4 border-l-[#00a2ff] hover:shadow-xl transition-all duration-300 group">
                            <div class="mb-4">
                                <p class="text-[10px] font-black text-[#00a2ff] uppercase tracking-widest mb-1"><?php echo htmlspecialchars($ep['tituloRueda']); ?></p>
                                <p class="text-base font-black text-gray-900 group-hover:text-[#00a2ff] transition-colors leading-tight"><?php echo htmlspecialchars($ep['contraparte']); ?></p>
                            ...
                            <button onclick="abrirModalEncuesta(<?php echo $ep['id']; ?>, '<?php echo addslashes(htmlspecialchars($ep['contraparte'])); ?>', '<?php echo addslashes(htmlspecialchars($ep['tituloRueda'])); ?>', '<?php echo date('d/m/Y H:i', strtotime($ep['fechaHora'])); ?>')"
                                    class="w-full bg-[#00a2ff] hover:bg-[#008ae0] text-white text-xs py-3 rounded-2xl font-black uppercase tracking-widest transition-all duration-300 shadow-md shadow-sky-500/10 flex items-center justify-center gap-2">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-[0_4px_25px_rgba(0,0,0,0.03)] border border-gray-100 overflow-hidden">
            <div class="bg-gradient-to-r from-[#008ae0] to-[#00a2ff] px-8 py-6">
                ...
                <?php else: ?>
                    <div class="space-y-6">
                        <?php foreach ($mis_encuestas as $enc): ?>
                            <div class="bg-white p-6 sm:p-8 rounded-[2rem] border border-gray-100 hover:border-sky-100 hover:shadow-xl transition-all duration-500 relative group">
                                <div class="flex flex-col md:flex-row md:items-start justify-between gap-6">
                                    <div class="flex-1">
                                        <div class="flex flex-wrap items-center gap-3 mb-2">
                                            <span class="text-[10px] font-black text-[#00a2ff] uppercase tracking-widest bg-sky-50 px-3 py-1 rounded-full border border-sky-100/50">
                                        ...
                                        <h3 class="text-lg font-black text-gray-900 mb-4 flex items-center gap-2 leading-tight">
                                            Reunión con: <span class="text-[#00a2ff]"><?php echo htmlspecialchars($enc['contraparte']); ?></span>
                                        </h3>
                                        ...
                                        <div class="flex flex-wrap items-center gap-4">
                                            ...
                                            <!-- Expectativa -->
                                            <div class="bg-sky-50/30 border border-sky-100 rounded-2xl p-3 flex items-center gap-3">
                                                <span class="text-[9px] font-black text-sky-600/60 uppercase tracking-widest">Expectativa</span>
                                                <span class="text-xs font-black text-[#00a2ff] uppercase">
                                        ...
                                        <?php if (!empty($enc['comentarios'])): ?>
                                            <div class="mt-6 p-5 bg-gray-50/80 rounded-2xl border-l-4 border-[#00a2ff] relative">
